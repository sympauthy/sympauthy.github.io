...
| ```identifier-claims```    | array of string | List of [claim](/documentation/technical/configuration#claims-id) identifiers that uniquely identify a person. These claims are used as the login identifier for password authentication and, when [`user-merging-enabled`](#auth) is `true`, as the key for merging accounts across authentication methods.                                   | NO<br>```[]```           |
...
| ```public```                | boolean         | When `true`, the client is a [public client](/documentation/functional/client#confidential-and-public-clients) that does not require a secret. Public clients must use [PKCE](/documentation/technical/security#pkce-proof-key-for-code-exchange) and cannot use the client credentials grant. | NO<br>```false```   |
...
| ```grant-unhandled-scopes```               | boolean | ⚠️ **UNSAFE - DEVELOPMENT ONLY**. Automatically grant ALL [grantable scopes](/documentation/functional/scope#grantable-scope) requested by the client that are not explicitly granted nor declined by any scope granting rule.<br>When enabled: Any grantable scope not explicitly granted nor declined is automatically granted.<br>When disabled: Grantable scopes not explicitly granted are rejected (secure default).<br>This feature only applies to grantable scopes. [Consentable scopes](/documentation/functional/scope#consentable-scope) always require end-user consent and [client scopes](/documentation/functional/scope#client-scope) have their own [granting rules](/documentation/functional/client_authorization#scope-granting-rules).<br>To know more about scope granting rules, see this [documentation](/documentation/functional/user_authorization#scope-granting-rules). | NO<br>```false```   |
...
| ```type```    | string  | The scope type. Either [`consentable`](/documentation/functional/scope#consentable-scope) or [`grantable`](/documentation/functional/scope#grantable-scope). Custom [`client`](/documentation/functional/scope#client-scope) scopes are not supported. | NO<br>```grantable```    |
