...
- **Skip guard**: The skip endpoint (`GET /api/v1/flow/mfa/skip`) is protected. When `mfa.required` is `true`,
  attempting to skip MFA returns an error, ensuring no user can bypass the requirement.
