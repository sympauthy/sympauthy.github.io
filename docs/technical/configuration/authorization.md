...
| ```identifier-claims```    | array of string | List of [claim](/technical/configuration/claim) identifiers a person signs in with. A user types a single value and it is matched against every claim listed, so a value must be unique across the whole list rather than within one claim: no two accounts may hold it under any of these claims, and no account may hold it under two of them. These claims are used as the login identifier for password authentication and, when [`user-merging-enabled`](#auth) is `true`, as the key for merging accounts across authentication methods. See [identifier claims](/functional/claims#identifier-claims).                                                 | NO<br>```[]```           |
| ```user-merging-enabled``` | boolean         | When `true`, accounts that share the same value for **every** configured `identifier-claims` are automatically merged across authentication methods (password, third-party providers); a partial match merges nothing. When `false`, each authentication method creates a separate account.                                                                                          | NO<br>```false```        |
